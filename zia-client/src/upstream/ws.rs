...
pub(crate) async fn transmit(
  ...
  let raw_upstream = Stream::connect(upstream, proxy).await?;
  let (upstream, _) = client_async(upstream, raw_upstream).await?;
  ...
  process_udp_over_ws(socket, upstream, Some(Duration::from_secs(60))).await;
